...
Use the same flow for `frontend` and `cli`: commit and push inside the submodule folder, then from the superproject run `git submodule update --remote <path>`, `git add <path>`, commit the pointer bump, and push.
